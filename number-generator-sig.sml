...
sig
  ...
  (* Create a slot with the given number in it.  Returns a reference to the
     status.  When Done, the caller owns the given slot.

     Note that you should release that slot before requesting additional
     numbers since the generator will reclaim unallocated slots with useful
     values.
  *)
